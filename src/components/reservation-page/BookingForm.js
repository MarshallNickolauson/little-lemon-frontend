...
function BookingForm({ onSuccess }) {
  ...
  useEffect(() => {
    if (formData.date) {
      ...
      const times = dayOfWeek === 5 || dayOfWeek === 6 ? weekendTimes : weekdayTimes;
  ...
  return (
    <div className='booking-form-container'>
      <form onSubmit={handleSubmit} className="booking-form">
        ...
        <label htmlFor="date">Choose ate</label>
        <input
          type="date"
          id="date"
          value={formData.date}
        ...
        <select id="time" value={formData.time} onChange={handleChange} disabled={!formData.date}>
        ...
        {!isFormValid && (
          <div className="error-message">Please complete all fields to submit</div>
        )}
